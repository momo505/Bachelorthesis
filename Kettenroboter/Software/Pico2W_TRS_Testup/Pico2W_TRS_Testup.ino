...
Adafruit_MCP2515 mcp(ADA_CS_PIN);
//*/
...
mutex_t  pb_mutex; // = xSemaphoreCreateMutex();
uint32_t pb_owner;
...
unsigned long c0_cycletime, c1_cycletime, ct_buf = 0;
...
void setup() {
    ...
    //Mutex für postbox
    initMutex(&pb_mutex);
    //serialmutex = createMutex();
    //mutexTake(serialmutex);
    if(&pb_mutex != NULL){
...
void setupCore1(){ // Core1: Motor control, QEI & Control loops
    ...
    //Serial.println("Serial switched to Core1");
    ...
    */
    ...
    //Serial.println("starting Motor-I2C");
    Wire.begin();
    //Serial.println("Motor-I2C should be up, Resuming Serial to Core0");
    //mutexGive(serialmutex);
...
void loop() {
    ...
    if(mutexTake(&pb_mutex, &pb_owner)){
        ...
        for(int i=0; i<2; i++){
            ...
            for(int ii=0; ii<7; ii++){
                Serial.print(" [ Value "); Serial.print(ii); Serial.print(": "); Serial.print(motor_c1_readvalues[i][ii]); Serial.print(" ] ");
            }
            Serial.println(" } ");
        }
        printvalues = false;
    ...
    Serial.print(" core0 cycle time: "); Serial.println((int)c0_cycletime);
    Serial.print(" core1 cycle time: "); Serial.println((int)ct_pb);
    ...
    c0_cycletime = millis() - c0_currentMillis; 
    }
...
void loopCore1(){ // Core1: Motor control, QEI & Control loops
    ...
    //if ((c1_currentMillis - c1_lastExecutedMillis >= 100)  || (singlecore==true) /* */) {
    ...
    */
    ...
    Wire.requestFrom(motor_add[j], 1);
    ...
    c1_cycletime = millis() - c1_currentMillis;
    /*
    }
    //*/
}
